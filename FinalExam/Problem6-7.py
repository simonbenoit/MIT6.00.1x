...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        while current_a != None or current_b != None:
            if current_a in visited_node:
                ancestor = current_a
                break
            elif current_a != None:
                visited_node.append(current_a)
                current_a = current_a.get_parent()
            if current_b in visited_node:
                ancestor = current_b
                break
            elif current_b != None:
                visited_node.append(current_b)
                current_b = current_b.get_parent()
    
        distance_node_a = self.get_distance(ancestor, node_a)
        distance_node_b = self.get_distance(ancestor, node_b)
        cousin_relationship = min(distance_node_a,distance_node_b)-1
        cousin_degree = max(distance_node_a,distance_node_b)-min(distance_node_a,distance_node_b)
        return (cousin_relationship, cousin_degree)
    
    
    
    def get_distance(self, ancestor, node):
        """Get the distance between a node and the ancestor"""
        distance = 0
        while node != ancestor:
            distance += 1
